...
void QExcel::saveAs(QString xlsFilePath)
{
    workBook->dynamicCall("SaveAs(Filename, FileFormat)", QDir::toNativeSeparators(xlsFilePath),
                          -4143);
}
...
void QExcel::setCellValue(int row, int column, const QVariant& value)
{
...
void QExcel::setCellValue(const QString& cell, const QVariant& value)
{
    QAxObject *range = sheet->querySubObject("Range(const QString&)", cell);
    range->dynamicCall("SetValue(const QVariant &)", value);
}
